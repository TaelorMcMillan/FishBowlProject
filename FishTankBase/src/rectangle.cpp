...
void rectangle::set(int text, double yt, double yb, double zcl, double zf, double xl, double xr)
{
...
void rectangle::drawFace(int i)
{
	...
	glEnable(GL_TEXTURE_2D);
		...
		if(textureID == 8) {
			...
			glVertex3f(vertex[face[i][3]][0], vertex[face[i][3]][1], vertex[face[i][3]][2]);
			glEnd();
		}
		else {
		glBegin(GL_QUADS);
...
bool rectangle::isFrontface(int faceindex, Camera camera) {
	GLfloat v[4];
