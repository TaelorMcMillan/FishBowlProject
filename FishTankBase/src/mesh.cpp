...
void Mesh:: readMesh(string fname)
{
  ...
  pt = new Point[numVerts];        assert(pt != NULL);
  norm = new Vector[numNorms];     assert(norm != NULL);
  ...
  for(int f = 0; f < numFaces; f++)
    {
      ...
      cout << n << "\n";
      for(int v = 0; v < n; v++)// write vertex indices for this face
	 cout << face[f].vert[v].vertIndex << " ";	cout << "\n";
      for(int k = 0; k < n; k++)	// write normal indices for this face
	cout << face[f].vert[k].normIndex << " "; cout << "\n";
    }
  cout << endl << flush;
}
